  ...
  addRecipe(){
    ...
    if(name && ingredients!.length>0 && instructions!.length>0 && prepTimeMinutes && cookTimeMinutes && servings && difficulty &&cuisine &&caloriesPerServing && image && mealType!.length>0){
      ...
      this.api.addRecipeAPI(this.recipeDetails).subscribe({
        next:(res:any)=>{
          // - if api call succes then clear all fileds, alert "recipe added" , redirect to all recipe page
        ...
        error:(reason:any)=>{
          // - if api call failed then alert failed message, clear name field of recipe details 
          alert(reason.error)
          this.recipeDetails.name = ""
        }
      })
    }else{
      // 4. if all value are not present then alert 'fill the form'
